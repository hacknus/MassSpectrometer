...
def nist_aprox(atom, integr_p, integr_p_err, plot_name, ethanol=False):
    ...
    if 29 and 43 and 41 in atom and ethanol != True:
        ...
        div = (
                    -but29 * iso41 * pro43 + but29 * iso43 * pro41 + but41 * iso29 * pro43 - but41 * iso43 * pro29 - but43 * iso29 * pro41 + but43 * iso41 * pro29)

        k_propane = -(
                    but29 * iso41 * p43 - but29 * iso43 * p41 - but41 * iso29 * p43 + but41 * iso43 * p29 + but43 * iso29 * p41 - but43 * iso41 * p29) / div
        k_propane_err = np.sqrt((but29 * iso41 * p43_err / div) ** 2 + (but29 * iso43 * p41_err / div) ** 2 + (
                    but41 * iso29 * p43_err / div) ** 2 + (but41 * iso43 * p29_err / div) ** 2 + (
                                            but43 * iso29 * p41_err / div) ** 2 + (but43 * iso41 * p29_err / div) ** 2)
        k_butane = -(
                    iso29 * p43 * pro41 - iso29 * p41 * pro43 - iso41 * p43 * pro29 + iso41 * p29 * pro43 + iso43 * p41 * pro29 - iso43 * p29 * pro41) / div
        k_butane_err = np.sqrt((iso29 * p43_err * pro41 / div) ** 2 + (iso29 * p41_err * pro43 / div) ** 2 + (
                    iso41 * p43_err * pro29 / div) ** 2 + (iso41 * p29_err * pro43 / div) ** 2 + (
                                           iso43 * p41_err * pro29 / div) ** 2 + (iso43 * p29_err * pro41 / div) ** 2)

        # k_iso = -(-but29 * p43 * pro41 + but29 * p41 * p3 + b2 * p43 * p1 - b2 * p29 * p3 - b3 *p41* p1 + b3 * p29 * pro41)
        k_isobutane = -(
                    -but29 * p43 * pro41 + but29 * p41 * pro43 + but41 * p43 * pro29 - but41 * p29 * pro43 - but43 * p41 * pro29 + but43 * p29 * pro41) / div
        k_isobutane_err = np.sqrt((but29 * p43_err * pro41 / div) ** 2 + (but29 * p41_err * pro43 / div) ** 2 + (
                    but41 * p43_err * pro29 / div) ** 2 + (but41 * p29_err * pro43 / div) ** 2 + (
                                              but43 * p41_err * pro29 / div) ** 2 + (
                                              but43 * p29_err * pro41 / div) ** 2)
    ...
    if 44 in atom:
        p_co2_scaled = (integr_p[atom == 44] - p_propane_scaled[amu_propane == 44] - p_isobutane_scaled[
            amu_isobutane == 44]) / p_co2[amu_co2 == 44] * p_co2
        p_co2_scaled_err = np.sqrt((integr_p_err[atom == 44] / p_co2[amu_co2 == 44] * p_co2) ** 2 + (
                    p_propane_scaled_err[amu_propane == 44] / p_co2[amu_co2 == 44] * p_co2) ** 2 + (
                                               p_isobutane_scaled_err[amu_isobutane == 44] / p_co2[
                                           amu_co2 == 44] * p_co2) ** 2)
        if p_co2_scaled[0] <= 0:
    ...
    if 28 in atom:
        p_n2_scaled = (integr_p[atom == 28] - p_co2_scaled[amu_co2 == 28] - p_butane_scaled[amu_butane == 28] -
                       p_ethanol_scaled[amu_ethanol == 28] -
                       p_propane_scaled[amu_propane == 28] - p_isobutane_scaled[amu_isobutane == 28]) / p_n2[
                          amu_n2 == 28] * p_n2
        p_n2_scaled_err = np.sqrt(
            integr_p_err[atom == 28] ** 2 + p_co2_scaled_err[amu_co2 == 28] ** 2 + p_butane_scaled_err[
                amu_butane == 28] ** 2 + p_propane_scaled_err[amu_propane == 28] ** 2 + p_ethanol_scaled[
                amu_ethanol == 28] ** 2 + p_isobutane_scaled[amu_isobutane == 28] ** 2) / p_n2[amu_n2 == 28] * p_n2
    else:
    ...
    # p  matrix:
    amu_gases = [amu_ethanol, amu_isobutane, amu_butane, amu_propane, amu_xenon, amu_argon, amu_krypton, amu_h2, amu_n2,
                 amu_h2o, amu_o2, amu_co2]
    p_gases = [p_ethanol_scaled, p_isobutane_scaled, p_butane_scaled, p_propane_scaled, p_xenon_scaled, p_argon_scaled,
               p_krypton_scaled, p_h2_scaled, p_n2_scaled, p_h2o_scaled, p_o2_scaled, p_co2_scaled]

    p_gases_err = [p_ethanol_scaled_err, p_isobutane_scaled_err, p_butane_scaled_err, p_propane_scaled_err,
                   p_xenon_scaled_err, p_argon_scaled_err, p_krypton_scaled_err, p_h2_scaled_err, p_n2_scaled_err,
                   p_h2o_scaled_err, p_o2_scaled_err, p_co2_scaled_err]
    ...
    for i in np.arange(len(M)):
        if sum(M[i]) != 0:
            M_rel[i] = M[i] / sum(M[i])
            if 1 not in M_rel[i]:
                M_rel_err[i] = np.sqrt((1 / sum(M[i]) ** 2 - 2 * M[i] / sum(M[i]) ** 3) * M_err[i] ** 2 + (
                            M[i] ** 2 / sum(M[i]) ** 4) * sum(M_err[i] ** 2))
    ...
    element = ['ethanol', 'isobutane', 'butane', 'propane', 'Xe', 'Ar', 'Kr', r'H$_2$', r'N$_2$', r'H$_2$O', r'O$_2$',
               r'CO$_2$']

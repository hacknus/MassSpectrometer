...
def scale_nist(atom,integr_p,gas,number=8):
    if gas == 'h2':
        amu = np.array([1,2])
        p = np.array([2,100])
    else:
...
def nist_aprox(atom,integr_p,integr_p_err,plot_name):
    ...
    if 29 in atom:
        p_propane_scaled = (integr_p[atom==29]-p_butane_scaled[amu_butane==29])/p_propane[amu_propane==29]*p_propane
    else: 
    ...
    if 28 in atom:
        p_n2_scaled = (integr_p[atom==28]-p_co2_scaled[amu_co2==28]-p_butane_scaled[amu_butane==28]-p_propane_scaled[amu_propane==28])/p_n2[amu_n2==28]*p_n2
    else:
    ...
    for i in amu_propane:
        if i in amu_butane:
            p_propane_scaled[amu_propane==i]=p_propane_scaled[amu_propane==i]+p_butane_scaled[amu_butane==i]
        if i in amu_h2o:
    ...
    if sum(p_krypton_scaled)!=0:
        ax.bar(amu_krypton-width/4,p_krypton_scaled, width=width/2, color="cyan",label=r'nist krypton')
    if sum(p_argon_scaled)!=0:
        ax.bar(amu_argon-width/4,p_argon_scaled, width=width/2, color="steelblue",label=r'nist argon')
    if sum(p_xenon_scaled)!=0:
        ax.bar(amu_xenon-width/4,p_xenon_scaled, width=width/2, color="deepskyblue",label=r'nist xenon')
    if sum(p_propane_0)!=0:
        ax.bar(amu_propane-width/4,p_propane_scaled, width=width/2, color="pink",label=r'nist $C_3H_8$')
    if sum(p_butane_0)!=0:
        ax.bar(amu_butane-width/4,p_butane_scaled, width=width/2, color="orange",label=r'nist $C_4H_{10}$')
    if sum(p_h2o_0)!=0:

...
#popt, pcov = curve_fit(oxygen, np.arange(0, 6), 1-co2rel, sigma=co2relerr, p0=popt)
#n = np.linspace(0, 6, 100)
#print(f"a = {popt[1]:.4f} +/- {np.sqrt(pcov[1][1]):.4f}")
#print(f"n0 = {100 * popt[0]:.4f} +/- {100 * np.sqrt(pcov[0][0]):.4f}")
...
plt.errorbar(np.arange(0,4), frac, frac_err, color="red", capsize=3, capthick=0.4, ecolor="black", elinewidth=0.4, fmt='.', label=r'O$_2$')
